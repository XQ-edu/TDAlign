...
def adjust_learning_rate(optimizer, scheduler, epoch, args, printout=True):
    ...
    elif args.lradj == "type7":  # only for MICN
        lr_adjust = {epoch: args.learning_rate * (0.75 ** ((epoch - 1) // 1))}
    elif args.lradj == "TST":  # for PatchTST
        lr_adjust = {epoch: scheduler.get_last_lr()[0]}
    elif args.lradj == "constant":  # only for PatchTST
        lr_adjust = {epoch: args.learning_rate}
    elif args.lradj == "type8":  # only for PatchTST
        lr_adjust = {
            epoch: (
                args.learning_rate
                if epoch < 3
    ...
    elif args.lradj == "type9":  # for SeqRNN
        lr_adjust = {
            epoch: (
                args.learning_rate
                if epoch < 3
